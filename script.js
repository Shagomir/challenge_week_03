...
function getLength() {
  ...
  while (l < 8 || l > 128) {
    l = prompt(
      "Please enter the length of the password required, between 8 and 128 characters",
      8
    );
  }
  return l;
...
function getParams() {
  ...
  while (!l && !u && !n && !s) {
    alert(
      "Please choose which characters to include in your password. Note that you must make at least one selection to proceed."
    );
    l = confirm(
...
function generateCharacters(pwdLength, param, charSet) {
  ...
  for (var i = 0; i < param.length; i++) {
    if (param[i]) {
      result += charSet[i].charAt(
        Math.floor(Math.random() * charSet[i].length)
      );
      allChars += charSet[i];
    }
  }
